...
All paths are computed from the package location — `config.py` resolves `Backend/` from `__file__` — so they no longer depend on the working directory the server was started from.
...
### Embedding Singleton (`models/embeddings.py`)
