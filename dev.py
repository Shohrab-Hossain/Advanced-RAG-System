...
Run from the repo root:
    python dev.py                  proxy mode  — frontend calls /api/* through the dev proxy
    python dev.py --direct         direct mode — frontend calls the backend cross-origin
    python dev.py --no-reload      skip the Flask reloader (models load once, boots ~2x faster)
    python dev.py --api-port 5000  pin a port instead of probing for one
...
# The backend runs with CWD=Backend/src because Config.DATA_ROOT ("./data") is
# resolved against the working directory — the live stores are in Backend/src/data.
...
def _check_backend_deps(python: str) -> None:
    ...
    if probe.returncode != 0:
        ...
        raise SystemExit(
            f"[dev] {python}\n"
            f"[dev] has no Flask — the backend would die on import. Install first:\n"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="dev.py", description="Run the RAG backend and frontend together.")
...
def main() -> int:
    ...
    for path in (BACKEND_SRC / "main.py", FRONTEND_DIR / "package.json"):
        if not path.exists():
            raise SystemExit(f"[dev] expected {path} — run dev.py from the repo root")
    ...
    _say(f"backend  {api_url}   ({BACKEND_SRC.name}/main.py)")
    ...
    try:
        api = _spawn([python, "main.py"], BACKEND_SRC, api_env)
